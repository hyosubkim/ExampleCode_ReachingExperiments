...
function sample_game(name_prefix, tgt_file_name_prefix, tgt_set)
...
% Load target file
cd('C:\Dropbox\extNFB\TargetFiles')
...
while trial <= maxtrialnum;   %
    ...
    hand_to_tgt_dist = sqrt((rcX - tgtx)^2 + (rcY - tgty)^2);
    ...
    if gamephase == 0   % Searching for start location
        ...
        %         if hand_dist < start_tolerance*2 && hand_dist >= (startcirclewidth/2 + cursor_r)
        %             visible = 0;
    ...
    elseif gamephase == 2;  % Moving towards target
        ...
        if hand_dist >= tgt_dist(trial,1)
            ...
            % check to see if cursor is touching target
            if hand_to_tgt_dist <= hit_tolerance
                tgt_touch = 1;
                tgt_touch_time = tgt_touch_time + dt;
            else
                tgt_touch = 0;
                tgt_touch_time = 0;
            end
            
            if tgt_touch_time >= end_movement_time
                gamephase = 3;
                tgt_touch_time = 0;
            end
    ...
    elseif gamephase == 4;  % Between Blocks Message
        ...
        if between_blocks(trial) ~= 0
            ...
            elseif between_blocks(trial) == 5
                Screen('DrawText', window, 'You will now get to familiarize yourself with the next condition.' , xCenter-480, yCenter, white);
                
            elseif between_blocks(trial) == 6
                Screen('DrawText', window, 'For the next practice trial, move your hand to the right.' , xCenter-500, yCenter, white);
                
            elseif between_blocks(trial) == 7
                Screen('DrawText', window, 'For the last practice trial, move your hand straight downwards.' , xCenter-500, yCenter, white);
